...
class gridPathFinder
{
	private:
		...
		std::multimap<double, GridNodePtr> openSet;	//multimap,<key,value>,按照key从小到大自动排序	

		//
		GridNodePtr start;
		GridNodePtr target;
		bool notReachTarget=true;
	...
	public:
		...
		//开始
		void astarGraphSearch(const Eigen::Vector3d start_pt, const Eigen::Vector3d target_pt);
		bool judeReachTarget(const Eigen::Vector3d pt_a,const Eigen::Vector3d pt_b);
		void expandNodes(GridNodePtr node);
